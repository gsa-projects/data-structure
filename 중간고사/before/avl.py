...
def balance(root: BSTNode) -> BSTNode:
    if bf(root) > 1:
        if bf(root.left) < 0:   # LR 문제
            root.left = rotate_left(root.left)  # TODO: 틀림. root = rotate_left(root) 라고 함
            root = rotate_right(root)
        else:   # LL 문제
            root = rotate_right(root)
    elif bf(root) < -1:
        if bf(root.right) > 0:  # RL 문제
            root.right = rotate_right(root.right)   # TODO: 똑같은 이유로 틀림
...
def insert(root: BSTNode, node: BSTNode) -> BSTNode | None:
    if root is None:
        node.height = 1     # TODO: 틀림. 이거 없으면 height가 제대로 안 더해짐
        return node
